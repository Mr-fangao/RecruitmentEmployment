...
export default {
  ...
  methods: {
    ...
    re() {
      ...
      this.word =
        "通过构建城市地理加权回归模型，分析我国招聘信息发布与地区生产总值、人均GDP、人均可支配收入等的关联。";
    },
    ying() {
      this.ceng = 2;
      if (this.ceng === 2) {
      ...
      this.word =
        "通过分析测算空间关联局域指标（Local Indicators of Spatial Association, LISA）显著水平，通过局部自相关分析方法，分析中国招聘信息发布的整体空间关联趋势，得出不同尺度范围下我国各区域招聘信息的关联和影响。";
    },
    jv() {
      this.ceng = 3;
      if (this.ceng === 3) {
      ...
      this.word =
        "新兴时空热点分析是GIS中的一种空间分析方法，能够根据某一段时间内已存在的地理分布数据做后续发展的时间和空间分析，通过新兴时空热点分析，获取招聘信息发布的热点和冷点趋势，分析我国招聘信息发布的时空演化特征，包括招聘信息数随时间变化的整体趋势、整体的范围、空间等级变化的整体趋势以及招聘重心随时间变化的整体趋势。";
    },
    feng() {
      this.ceng = 4;
      if (this.ceng === 4) {
      ...
      this.word =
        "分别计算 x 和 y 方向上的标准距离，用于定义一个包含所有要素分布的椭圆的轴，用于展示空间要素在空间分布中的趋势，用以反映中国招聘信息的空间分布和移动特征。";
    },
    san() {
      this.ceng = 5;
      if (this.ceng === 5) {
