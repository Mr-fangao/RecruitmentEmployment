        ...
        <el-tabs v-model="activeName" type="border-card">
          ...
          <el-tab-pane label="县区" name="third">
            ...
            <ul v-if="xians.length !== 0">
              <li v-for="(value, item) in xians" :key="item" class="publiz" @click="xiansD(value)">
                {{ value.name }}
              </li>
            </ul>
          </el-tab-pane>
...
export default {
  ...
  created() {
    var that = this
    that.citys = JSON.parse(window.localStorage.getItem('city')) // 读取值
    that.json.name = that.mapdata.name
    that.json.where = that.mapdata.where
    if (that.json.name !== '中国') {
      setTimeout(function() {
        eventBum.$emit('json', that.json)
      }, 500)
    }
  },
  beforeDestroy() {
    this.mapdata.name = this.json.name
    this.mapdata.where = this.json.where
  },
  mounted() {
    var that = this
